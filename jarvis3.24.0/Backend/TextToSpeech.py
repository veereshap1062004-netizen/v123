...
from dotenv import dotenv_values  # Fixed: import statement
...
def TTS(Text, func=lambda r=None: True):
    while True:
        try:
            ...
            print(f"Error in TTS: {e}")  # Fixed: added space after colon
            return False
        finally:
            try:
                func(False)
...
def TextToSpeech(Text, func=lambda r=None: True):
    ...
    if len(Data) > 4 and len(Text) >= 250:
        # Fixed: was " ,join" and incorrect slicing
        first_part = ".".join(Data[0:2]) + "." + random.choice(responses)
        TTS(first_part, func)
